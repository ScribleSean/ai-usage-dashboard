...
- **Interface:** Mac empty-state, keyboard-direction, selection-feedback and minimum-window spot checks are recorded in [native UI verification](UI-VERIFICATION.md). Complete populated-data, full keyboard, enlarged-text and Windows-native checks. A rendered view or source-level test is not a full accessibility review.
