...
- **Interface:** Mac empty-state, keyboard-direction and minimum-window spot checks are recorded in [native UI verification](UI-VERIFICATION.md). Resolve the remaining visual selection highlight and complete populated-data, full keyboard, enlarged-text and Windows-native checks. A rendered view or source-level test is not a full accessibility review.
