...
- **Interface:** Mac empty-state, keyboard-direction, selection-feedback, minimum-window and 200% page-zoom spot checks are recorded in [native UI verification](UI-VERIFICATION.md). Complete populated-data, full keyboard and Windows-native checks, including enlarged layouts. A rendered view or source-level test is not a full accessibility review.
