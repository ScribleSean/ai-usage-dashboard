...
export function demoData() {
  ...
  return {demo:true,schema:2,timezone:'America/New_York',collectedAt,
    dictation:['Mac','Windows'].map((host,h)=>({host,status:'ok',checkedAt:collectedAt,calendar:'device-local',scope:'retained-transcriptions',days:dates.filter((_,i)=>i%4!==0).map((date,i)=>({date,transcriptions:3+i+h,words:42*(i+1+h),audioSeconds:20*(i+1+h),engines:[{engine:h?'Parakeet / sherpa-onnx':'Apple Speech',transcriptions:3+i+h}]}))})),
